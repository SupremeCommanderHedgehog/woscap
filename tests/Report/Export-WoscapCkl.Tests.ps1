...
Describe 'Export-WoscapCkl' {
    It 'writes a well-formed legacy CKL with mapped VULN entries' {
        InModuleScope woscap {
            $results = @(
                New-WoscapResult -StigId 'WN11-00-000165' -GroupId 'V-253283' -GroupTitle 'SRG-OS-000480' -RuleId 'SV-253283r1_rule' -Severity 'medium' -Title 'SMB1 & <disabled>' -Result 'Fail' -Observed 1 -Expected 0 -Cci @('CCI-000366','CCI-000999') -ComputerName 'PC1' -Benchmark 'Microsoft Windows 11 STIG' -BenchmarkVersion '2' -CheckText 'reg check' -FixText 'disable smb1' -Discussion 'why' -FindingDetails 'found 1'
            ...
            try {
                ...
                ($v.STIG_DATA | Where-Object VULN_ATTRIBUTE -eq 'Group_Title').ATTRIBUTE_DATA | Should -Be 'SRG-OS-000480'
                ...
                $pass = $vulns | Where-Object { ($_.STIG_DATA | Where-Object VULN_ATTRIBUTE -eq 'Rule_Ver').ATTRIBUTE_DATA -eq 'WN11-AU-000500' }
                $pass.STATUS | Should -Be 'NotAFinding'
                # No XCCDF group title -> empty Group_Title, never the rule title (issue #36).
                ($pass.STIG_DATA | Where-Object VULN_ATTRIBUTE -eq 'Group_Title').ATTRIBUTE_DATA | Should -Not -Be 'Log'
